...
const tableRows = computed<ConnectionRow[]>(() =>
	paginatedConnections.value.map((connection) => {
		const providerTypeInfo = getProviderTypeInfo(connection.type);
		...
		return {
			...
			connectionDisplayName: providerTypeInfo?.displayName ?? connection.type,
...
const tableHeaders = computed<Array<TableHeader<ConnectionRow>>>(() => [
	...
	},
	{
		...
		disableSort: true,
		resize: false,
		value: (row: ConnectionRow) => row.connectionDisplayName,
	},
]);
			...
			>
				<template #item="{ item }">
					<tr :class="$style.groupHeaderRow">
						<td colspan="2" :class="$style.groupHeaderCell">
					...
					<tr v-if="item.isExpanded && item.secrets.length > 0">
						<td colspan="2" :class="$style.secretsCell">
...
.secretRow {
	...
	align-items: center;
	padding: var(--spacing--2xs) var(--spacing--md) var(--spacing--2xs) var(--spacing--xl);
